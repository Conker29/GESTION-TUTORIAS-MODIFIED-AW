...
const registrarDocente = async (req, res) => {
  try {
    ...
    if (req.files?.imagen) {
      //console.log("Archivo recibido en req.files.imagen:", req.files.imagen);
      const { secure_url, public_id } = await cloudinary.uploader.upload(req.files.imagen.tempFilePath, { folder: "Docentes" });
      console.log("Imagen subida con éxito:", secure_url);
    ...
    //Verificar si se guardó la imagen en MongoDB: console.log("Docente guardado en MongoDB:", nuevoDocente);
...
const listarDocentes = async (req,res)=>{
      if (req.docenteBDD?.rol ==="Docente"){
        const docentes = await Docente.find(req.pacienteBDD._id).select("-salida -createdAt -updatedAt -__v").populate('veterinario','_id nombre apellido')
        res.status(200).json(pacientes)
    }
    else{
        const docentes = await Docente.find({estadoDocente:true}).where('administrador').equals(req.administradorBDD).select("-salida -createdAt -updatedAt -__v").populate('administrador','_id nombre apellido')
...
const detalleDocente = async(req,res)=>{
    const {id} = req.params
    if( !mongoose.Types.ObjectId.isValid(id) ) return res.status(404).json({msg:`Lo sentimos, 
      no existe el registro del docente ${id}`});
    const docente = await Docente.findById(id).select("-createdAt -updatedAt -__v")
