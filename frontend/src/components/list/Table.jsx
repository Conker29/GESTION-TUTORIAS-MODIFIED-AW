...
const Table = () => {
  ...
  const listDocentes = async () => {
    ...
    if (response) setDocentes(response.docentes || []);
  };
  ...
  // Función para asegurarse que asignaturas sea un array y no un string JSON
  const parseAsignaturas = (asignaturas) => {
    if (Array.isArray(asignaturas)) return asignaturas;
    if (typeof asignaturas === "string") {
      try {
        const parsed = JSON.parse(asignaturas);
        if (Array.isArray(parsed)) return parsed;
      } catch (e) {
        // Si no puede parsear, retorna el string dentro de un array para que no crashee
        return [asignaturas];
      }
    }
    return [];
      ...
      <table className="w-full mt-5 table-auto shadow-lg bg-white rounded-lg overflow-hidden">
        ...
        <tbody className="text-gray-700 text-sm">
          {docentes.map((docente, index) => {
            const asignaturasArray = parseAsignaturas(docente.asignaturas);
            return (
